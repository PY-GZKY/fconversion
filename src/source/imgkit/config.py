...
from src.source.imgkit.utils import get_configuration_
from src.source.pdfkit.utils import wkhtmltoimage_error


class Config:
    """Config class to configure wkhtmltoimage, xvfb-run and meta tag prefix"""

    def __init__(self, wkhtmltoimage=None, xvfb="", meta_tag_prefix="imgkit-"):
        """
    ...
    def get_wkhtmltoimage(self):
        ...
        if not self.wkhtmltoimage:
            ...
            self.wkhtmltoimage = get_configuration_()
    ...
    def get_xvfb(self):
        ...
        if not self.xvfb:
            ...
            for find_cmd in ("where", "which"):
                try:
                    self.xvfb = (
                        subprocess.check_output([find_cmd, "xvfb-run"]).strip().decode("utf-8")
                    )
                    break
